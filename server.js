...
// ✅ CORS (GitHub Pages/브라우저에서 Render API 호출 허용)
...
/* ==========================================================
   ✅ BINANCE KLINES (Browser-safe)
   - 브라우저에서 Binance 직접호출 CORS 막힘 → 서버가 대신 호출
   ========================================================== */
app.get("/api/bn/klines", async (req,res)=>{
  try{
    const symbol = String(req.query.symbol||"").toUpperCase().trim();
    const tfRaw = String(req.query.tf||req.query.interval||"").trim();
    const limit = Number(req.query.limit||500);
    if(!symbol || !tfRaw) return res.status(400).json({ ok:false, error:"BAD_QUERY", detail:"symbol/tf required" });
    const candles = await _fetchKlinesFut(symbol, tfRaw, limit);
    return res.json({ ok:true, symbol, tfRaw, candles });
  }catch(e){
    return res.status(500).json({ ok:false, error:"BN_KLINES_FAIL", detail:e?.message||String(e) });
  }
});
